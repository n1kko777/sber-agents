...
async def open_deposit(
    ...
    rate: Annotated[
        float,
        Field(
            description="Фиксированная ставка по вкладу (годовых)",
            ge=0.1,
            le=100,
            examples=[14.5, 17.0]
        )
    ...
    capitalization: Annotated[
        bool,
        Field(description="Капитализировать проценты ежемесячно", examples=[True, False])
    ] = True
) -> str:
    """
    ...
    """
    logger.info(f"🔐 open_deposit called: client={client_name}, amount={amount}, rate={rate}, term={term_months}")
    ...
    result = (
        ...
        f"   Капитализация: {'ежемесячно' if capitalization else 'нет'}\n"
