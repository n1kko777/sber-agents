...
- Разбивает на чанки ~450 символов с overlap 120
- Подмешивает Q&A из `data/sberbank_help_documents.json`
...
## 🧪 Локальное тестирование RAG

Чтобы быстро проверить покрытие ключевых вопросов без запуска Telegram, запустите:

```bash
uv run python src/local_test.py
```

Скрипт прогонит индексацию (PDF + JSON) и покажет топ-чанки для контрольных вопросов:
...
- `-q/--question` — добавить свои вопросы (флаг можно повторять)
- `--top-k` — сколько чанков показывать на вопрос
- `--ask-llm` — дополнительно получить полный ответ от RAG-цепочки (потребуется настроенный LLM)
