...
- 🧠 **Самостоятельное принятие решений** - агент сам выбирает когда искать информацию
- 🔄 **ReAct паттерн:**
  - **Reason (Think)** - анализирует вопрос и контекст
  - **Act** - вызывает инструмент `rag_search` при необходимости
  - **Respond** - формирует ответ на основе найденных данных
- 💾 **MemorySaver** - автоматическое сохранение истории диалогов
- 🎯 **Умная стратегия:**
  - Простые вопросы ("привет", "спасибо") → отвечает напрямую
  - Вопросы о продуктах → использует поиск в документах
...
- `/graph` - Показать граф вызовов инструментов для последнего запроса
